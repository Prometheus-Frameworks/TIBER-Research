...
discovery under TIBER-Research issue #5. Read-only; no maintainer contact, no
issues/PRs opened anywhere.
...
logs for nflverse repos were not retrievable; claims are pinned to (a)
...
pkgdown/CRAN pages, and (d) issue/PR pages. Everything marked **[inference]**
is a reading, not an observed statement.

## 1. Repository map — who owns which availability-adjacent surface

Authoritative mapping source: the nflverse "Data Update and Availability
...
| Surface | Automation repo / workflow | Schedule (observed cron) | Output artifact | Freshness observed 2026-08-13 |
|---|---|---|---|---|
| Rosters (season + weekly) | `nflverse/nflverse-rosters` → `update_rosters.yaml` | `0 7 * * *` daily, year-round | tags `rosters`, `weekly_rosters` | `2026-08-12 04:12:57 EDT` → **active** |
| Depth charts | `nflverse-rosters` → `update_depth_charts.yaml` → `exec/update-depth-charts.R` | `0 7 * * *` daily | tag `depth_charts`, cumulative `depth_charts_{season}` keyed by ISO8601 `dt` | `2026-08-12 04:11:49 EDT` → **active** |
...
| Players / IDs | `nflverse/nflverse-players` → `update_players.yaml`; builders `players_espn_release()`, `players_pfr_release()`, `players_otc_release()`; manual overrides JSON | daily | tag `players` (v2 as of nflreadr 1.5.0) | active per status table |
| Release/distribution | `nflverse/nflverse-data` — release buckets only, no scraping code; data licensed **CC-BY 4.0** (`LICENSE.md`) | n/a | ~24 release tags, each with `timestamp.json` | — |
| ESPN QBR | `nflverse/espnscrapeR-data` → `update_qbr.yaml` (cron `0 9 * 1,2,9-12 *`) using `jthomasmock/espnscrapeR` | daily 09:00 UTC in-season | tag `espn_data` | `2026-02-28 04:17:38 EST` — idle in August, consistent with cron |
| NGS (context) | `nflverse/ngs-data` → `update_ngs.yaml`, cron `0 7 * 1,2,9-12 *` | in-season nightly | tag `nextgen_stats` | active per status table |
| PFR snap counts (context) | `nflverse/pfr_scrapR` → `update_snap_counts.yaml`, in-season 4×/day | in-season | tag `snap_counts` | active per status table |

**Public/private split (observed via unauthenticated 404s, 2026-08-13):** the
*automation orchestration* repos are public (`nflverse-rosters`,
`nflverse-players`, `pfr_scrapR`, `ngs-data`, `espnscrapeR-data`,
`nflverse-pbp`), but the *scraper packages that touch upstream endpoints* are
**private**: `nflverse/nflapi` (NFL endpoints; injuries and rosters),
...
**[Inference]** keeping endpoint-touching code private is deliberate posture;
no maintainer statement observed saying so.

## 2. Depth-chart precedent — verified, with corrections
...
- **Current pipeline** (branch head of `exec/update-depth-charts.R`,
  ...
  **private `nflverse/nflverse-espn`**, so the exact ESPN endpoint is not
...
**Precedent shape (observed):** original licensed/official source dies →
maintainers stand up an ESPN-derived replacement in a *private* scraper
...
- **ESPN Total QBR is distributed by nflverse today** via GitHub-Actions
...
- The same repo's `update-fpi-pbwr.yaml` (ESPN FPI and pass-block/pass-rush
  win rates via `rvest`) is **manual-dispatch only**, committing CSVs to the
  repo — a lower-ceremony, non-release-grade ESPN feed coexists with the
  release-grade QBR one.
- `espn_id` is a first-class identifier in the players dataset; ESPN is
  in-scope in `nflverse-players` CONTRIBUTING.
- **Recorded maintainer position on ESPN rights/takedown risk: none found.**
  No maintainer statement on ESPN ToS, redistribution rights, or takedown
  exposure anywhere in the observed record; nflverse-data has no GitHub
  Discussions. The observable posture is behavioral: ESPN-touching scraper
  code moved into a private repo while ESPN-derived *data* ships publicly
  under CC-BY. Contrast: explicitly negotiated sources state their terms (FTN
  participation/charting: "graciously offered/provided," CC-BY-SA 4.0, credit
  "FTN Data via nflverse"). **[Inference]** ESPN-derived data in nflverse
  rests on tolerated scraping of public endpoints, not a documented grant.
...
- **How datasets historically entered (observed cases):**
  ...
  - *Outside-contributor units smaller than a dataset are common* and
    ...
    **No observed case of an outside contributor landing an entirely new
    scraped dataset end-to-end**; new *sources* have entered via maintainers
    or provider partnerships.
- **Schema/dictionary expectations:** every distributed dataset gets a
  `dictionary_*` in nflreadr; source changes get dictionary notes. Artifacts
  ship rds/csv/parquet with per-tag `timestamp.json`; saves route through
  `nflversedata::nflverse_save()`.
- **Versioning:** release tags updated in place (append or overwrite per
  dataset), not semver; breaking data changes gate to loader CRAN releases
  timed to season start (1.4.0 → Sep 2023; 1.5.0 → Sep 2025), documented in
  NEWS.

## 5. What precedent does NOT establish

1. **ESPN-derived depth charts and QBR existing in nflverse does not
   establish that ESPN grants redistribution rights to anyone** — for
   injuries or anything else. No ESPN license, permission, or maintainer
   rights-position statement was found.
2. **nflverse's CC-BY 4.0 on releases licenses only what the maintainers can
   license; it does not cleanse upstream rights.** Where nflverse *does* have
   negotiated terms (FTN), it says so and imposes CC-BY-SA + attribution;
   ESPN data carries no such statement.
3. **Maintainer acceptance of a source is not transferable to third
   parties.** That nflverse scrapes ESPN from its own private repo says
   nothing about whether TIBER may scrape ESPN or build "the same thing" for
   injuries.
4. **A dataset's continued availability is not a commitment.** NFL Data
   Exchange (depth charts) and NGS participation both died with no recourse;
   the injuries vignette said "no ETA"; injuries data later reappeared
   silently with no doc update.
5. **The depth-chart precedent does not establish schema stability or
   drop-in replacement.** The accepted ESPN feed shipped a fully breaking
   schema and changed semantics.
6. **r-universe distribution of community tooling (espnscrapeR) does not
   imply org endorsement of its legal posture.**
...
- **Injuries:** vignette states "Our data source died after the 2024 season.
  ...
  for issue #5:** the `injuries` release now contains a full-season
  `injuries_2025.csv` (downloaded and verified 2026-08-13: 6,068 rows, REG
  1–18 + POST 19–22, all 32 teams, all `gsis_id`s populated; legacy schema
  plus `season_type`, minus `date_modified`), and `injuries/timestamp.json`
  reads `2026-03-18 08:45:29 EDT`. 2025 injuries were **retroactively
  backfilled around 2026-03-18**, months after the season, while docs still
  say "no 2025 data." The script calls private `nflapi::nflapi_injuries()`;
  the cron (Sep–Feb) is enabled; whether 2026 updates *in-season* is
  **unknown** (the March backfill was outside the cron window — **[inference]**
  a manual `workflow_dispatch`). The open gap is *in-season 2026 latency*, not
  2025 existence.
...
- `exec/update_roster.R` aborts on empty season rosters but only
  ...
  secondary artifact.
- Per-tag `timestamp.json` + shields.io badge tables are the alerting
  surface; no paging/webhook alerting observable. An archived
  `nflverse/status` repo (archived Mar 2022) shows a dedicated status page
  was abandoned in favor of the badge table.
- `heartbeat.yml` workflows (monthly no-op commits) exist solely to stop
  GitHub from auto-disabling scheduled workflows — deliberate cron-liveness
  engineering.
- Season-scoped crons mean "no recent run" is normal out of season; any
  monitoring design against nflverse must key off `timestamp.json` +
  calendar, not run recency.

**Net reliability model [inference]:** fail closed at the artifact level,
degrade honestly in docs (usually), no upstream continuity guarantees, replace
dead sources opportunistically on maintainer/partner initiative, tolerate
multi-month dark windows, communicate through badges, NEWS, dictionaries, and
Discord rather than formal deprecation.
