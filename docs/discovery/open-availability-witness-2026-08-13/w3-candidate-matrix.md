# W3 — Candidate-source matrix for 2025+ NFL availability evidence

Provenance: dated agent discovery reference, 2026-08-13, prepared under the
investigation-and-design scope of TIBER-Research issue #5. The authority and
custody boundary in [`README.md`](README.md) controls this file.

**Method note:** all observations are from 1–2 polite sample requests per
endpoint made on 2026-08-13, read-only, no credentials, no pagination sweeps,
no gate bypass. robots.txt / ToS revision dates are as observed that day. Raw
sample payloads were retained only in ephemeral session scratch space for
shape documentation and are **not** committed to this repository.

---

## Candidate 1a — NFL official communications (nflcommunications.com → media.nfl.com)

- **Source owner:** NFL Enterprises LLC.
- **Identity / access:** `nflcommunications.com` **301-redirects to
  `media.nfl.com`** (Adobe AEM Cloud). Weekly injury-report PDFs exist at
  paths like `…/content/dam/communications/media/injury-reports/2025/Week 17
  Injury Report - Wednesday.pdf`.
- **Auth/gating:** **HTTP 401 on the PDF path** (observed). Registration is
  offered only "for members of news organizations covering the NFL on-site
  regularly." Publicly visible sections do not include the weekly injury
  reports.
- **robots.txt:** `nflcommunications.com/robots.txt` 302 →
  `mediaarchive.nfl.net/robots.txt` → **403**; `media.nfl.com/robots.txt` →
  **500** at check time. No robots policy retrievable.
- **ToS:** NFL Terms of Service (same instrument as 1b).
- **Coverage:** the gated PDFs are the canonical league-distributed per-day
  (Wed/Thu/Fri) practice reports and game-status reports — the richest
  official artifact class.
- **Identity quality:** PDFs; names only, no machine IDs.
- **Clocks:** filed per league policy ("clubs must file Practice Reports by
  4:00 p.m. New York time"); per-day editions are themselves the clock.
- **Failure modes:** platform migration already happened once; AEM
  instability observed.
- **Discovery disposition — not an admission record:** excluded from this
  discovery envelope because the required media credentials were unavailable.
  No admission or broader rights decision is made.

## Candidate 1b — NFL.com public injuries pages (`www.nfl.com/injuries/…`, `/inactives/`)

- **Source owner:** NFL Enterprises LLC.
- **Identity / access:** server-rendered public HTML, no auth:
  - Current week: `https://www.nfl.com/injuries/` (200; "Week 1 of the 2026
    Season"; empty tables in preseason).
  - Historical weeks: `…/injuries/league/{season}/{TYPE}{week}` — sampled
    `/injuries/league/2025/REG18` → 200, 415 table rows, columns
    **Player | Position | Injury (body parts) | Practice status | Game
    status**. ⚠ Routing anomaly: the REG18 URL served a page titled "Week 1
    of the 2025 Season" — the week parameter may be partially ignored or the
    title stale; verify before any use.
  - Gameday inactives: `https://www.nfl.com/inactives/` (200; empty outside
    gamedays) — NFL.com publishes league gameday inactives, a class no other
    evaluated open source carried in this audit.
  - Backing API: no visible public JSON endpoint in served HTML; the known
    `api.nfl.com` requires an OAuth client token — treated as gated, not
    probed.
- **robots.txt** (observed 2026-08-13): `User-agent: *` disallowed only
  `/_ctv/, /_fantasy-app/, /_libraries/, /_mobile-app/, /_mobileview/, /_phs/,
  /_sponsors/, /account/, /nfl-films-beta/, /search/`. **`/injuries/` and
  `/inactives/` were not disallowed; no AI-agent blocks were observed. This is
  not described as permission.**
- **ToS** (`https://www.nfl.com/legal/terms`, "Last Updated: May 16, 2024"):
  - §1.3: *"Systematic retrieval of data or other content from the Services,
    whether to create or compile, directly or indirectly, a collection,
    compilation, database, or directory, is prohibited absent our express
    prior written consent."*
  - §1.3: *"You may use the Services solely for your own individual
    non-commercial and informational purposes only."*
  - §11: prohibits *"any engine, software, tool, agent or other device or
    mechanism (including, browsers, spiders, robots, avatars or intelligent
    agents) to navigate or search the Services to harvest or otherwise
    collect information from the Services to be used for any commercial
    purpose."*
- **Coverage:** practice status (weekly aggregate on the HTML page, not
  per-day) ✔; official game designation ✔; gameday inactives ✔ (separate
  page); IR/PUP as such ✖ (transactions, not on the report page).
- **Identity:** display names + team only; no gsis in markup; name→gsis
  resolution needed (nflverse rosters make it tractable; injury-report name
  collisions are a known failure mode).
- **Clocks:** none in the served HTML.
- **Backfill:** the sampled 2025 week page was live; neither page completeness
  nor archive depth was established.
- **Schema stability:** plain HTML table; page redesigned before; the routing
  anomaly is itself a stability warning.
- **Verdict:** league-published content, and the ToS systematic-retrieval
  clause on its face describes exactly what a pipeline does. Whether the
  underlying facts are protectable, and whether the browsewrap is
  enforceable against fact extraction, are **counsel questions this audit
  does not adjudicate** (routed in the unresolved-questions section).
  **Discovery disposition — not an admission record:** no admission proposal
  is made; a governed review would need to resolve the rights question. Raw
  HTML was not retained; public reachability ≠ permission.

## Candidate 2 — First-party club sites (sampled: Philadelphia Eagles)

- **Owner:** Philadelphia Eagles, LLC; page served on the NFL club-platform
  CMS. This discovery sampled the Eagles site, not all 32 clubs.
- **Identity:** `…/team/injury-report/` — 200, public, with an empty legend-only
  shell in the sampled preseason response. This pass did not observe an
  in-season page or establish its population.
- **robots.txt:** the observed text disallowed `/search/`, `/_libraries/`, and
  `/_mobileview/`; the sampled injury path was not disallowed. This is not
  described as permission.
- **ToS** (`…/footer/terms-conditions`, "Last Updated: June 1, 2026"):
  **verbatim the same NFL-platform boilerplate** — systematic-retrieval
  prohibition, personal non-commercial use, identical spider/robot clause.
  The Eagles sample and a prior Cardinals audit showed matching
  league-platform boilerplate; no 32-site census was performed.
- **Discovery disposition — not an admission record:** excluded from this
  comparison pass because the sampled source is dominated by the league page;
  its rights and admission questions remain unresolved.

## Candidate 3 — ESPN site API (`site.api.espn.com`)

- **Owner:** ESPN Inc. / The Walt Disney Company.
- **Sample:** `GET …/apis/site/v2/sports/football/nfl/injuries` → 200, JSON,
  **8.94 MB**, no auth. The response contained 32 distinct team entries; that
  ledger count is not a declared-universe or completeness claim. Team-scoped
  variants were also observed.
- **Shape (observed):** top level `{timestamp, status, season, injuries[32]}`;
  `timestamp: "2026-08-13T03:09:18Z"`; per item: `id, longComment,
  shortComment, status, date, athlete, source, type{…}, details{fantasyStatus,
  type, location, detail, side, returnDate}`.
  - `status` domain observed: `Active, Questionable, Out, Injured Reserve,
    Suspension`.
  - `details.fantasyStatus` domain observed: `QUESTIONABLE, OUT, IR, PUP-P,
    NFI-R, NFI-A, RESERVE-SUS` — **carries IR/PUP/NFI reserve-list classes**,
    which NFL.com's report page does not.
  - `details.type` = 30 body-part values incl. `Knee - ACL`, `Concussion`.
  - Per-item `date` to the minute plus `details.returnDate`.
- **Load-bearing caveat:** `longComment`/`shortComment` are **editorial
  fantasy-news prose with reporter attribution** — expressive editorial
  content, categorically different in kind from normalized status fields.
  None was retained in this discovery pass; protectability is a counsel
  question. No official per-day practice participation; no gameday
  inactives; `status` is ESPN's synthesis, not the league report.
- **Auth/robots/ToS:** no auth. `site.api.espn.com/robots.txt` → **403 (none
  served)**. The endpoint is **undocumented; no affirmative license was
  observed** (ESPN's public developer program shut down in the mid-2010s).
  The terms instrument reviewed for the Disney/ESPN property was the Disney
  Terms of Use (`disneytermsofuse.com/english/`, "Last Updated: May
  24, 2024"): prohibits *"access, monitor, copy or extract the Disney
  Products using a robot, spider, script, or other automated means"*, any
  *"creating or developing any AI Tool, data mining or web scraping"*, any
  *"use, creation, development, modification, prompting, fine-tuning,
  training… of any artificial intelligence or machine learning tool"*, any
  *"commercial or business-related use"*, and redistribution. The
  automated-access exception applies only where *"not disallowed by Disney,
  including through the applicable robots.txt"* — and `www.espn.com/robots.txt`
  (observed 2026-08-13) explicitly blocks AI agents (`anthropic-ai`, GPTBot,
  Google-Extended, CCBot, Bytespider, ChatGPT-User…). The API hosts served no
  robots file, so no affirmative automation allowance was observed there.
  Applicability of that terms instrument to these API hosts remains unresolved.
- **Identity:** ESPN athlete id; crosswalk to gsis via nflverse
  `ff_playerids` — feasible, good coverage for rostered skill players.
- **Backfill:** none — current-state snapshot; history would require
  archiving it yourself, which is the systematic-collection pattern the ToU
  text facially prohibits.
- **Stability:** the endpoint is undocumented and the one-day sample does not
  establish a stability contract or long-run behavior.
- **Verdict:** technically the **best open-reachability signal with real
  clocks**, but the most restrictive terms text observed in this audit.
  **Discovery disposition — not an admission record:** no admission proposal
  is made. The editorial comment fields are
  expressive prose rather than normalized status fields and none was retained
  in this pass; protectability, enforceability, and implied-license questions
  are routed to counsel, not answered here.

## Candidate 4 — ESPN core API (`sports.core.api.espn.com`)

- **Samples (2):** team injuries collection (paginated `$ref` list;
  `count: 80` for team 22 — includes shallow in-season history) + one item
  deref → **identical payload to the site-API item**, with `athlete`/`team`
  as `$ref` URLs.
- **Differences vs #3:** N+1 fetch pattern — strictly **worse politeness
  economics** (hundreds of requests per league snapshot vs one); slight plus:
  per-athlete per-season history. robots → 403 (none served).
- **Verdict:** same owner and observed ToU; no admission proposal is made;
  dominated by #3 on every axis except per-athlete history. Used only as a
  discovery schema cross-check.

## Candidate 5 — Sleeper players dump (`api.sleeper.app/v1/players/nfl`)

- **Owner:** Sleeper (Blitz Studios, Inc.).
- **Sample:** one GET → 200, **14.64 MB**, 12,218 players, no auth/key.
- **Docs** (`docs.sleeper.com`, observed 2026-08-13): free, read-only, no
  token. *"stay under 1000 API calls per minute, otherwise, you risk being
  IP-blocked."* Players endpoint: *"use this call sparingly, as it is
  intended only to be used once per day at most"*. Licensing: free *"for
  non-commercial purposes"*; *"for commercial use, please reach out to us
  directly to discuss licensing"*; attribution requested for trending data.
- **robots.txt:** `api.sleeper.app` returned a placeholder and `sleeper.com`
  returned `Allow: /`; these observations are not described as permission.
- **ToS** (`support.sleeper.com/…/terms-of-use`, "Last Updated: July 24,
  2026"): §9.2 grants a *"limited, personal, revocable, non-transferable and
  non-exclusive right… for your personal and non-commercial use"*; no
  rent/lease/sell/redistribute/sublicense/copy; §2.9: no transfer of any
  right, title, or interest in *"any data, content, or intellectual property
  therein."* No explicit anti-scraping clause for data extraction.
- **Injury fields observed (2026-08-13 dump):**
  - `injury_status` domain + counts: `Questionable 359, NA 91, IR 73, PUP 56,
    Out 7, Sus 6, COV 2, DNR 2` (596 flagged). No `Doubtful` observed in this
    preseason snapshot; Sleeper collapses to its own vocabulary.
  - `injury_body_part`: 43 values — same taxonomy family as ESPN's.
  - `practice_participation`/`practice_description`: **effectively
    unpopulated** (only `Out` / `Out (Definitely Will Not Play)` observed).
    Not a practice-report witness, at least in preseason.
  - `injury_start_date`: **0 of 596 populated.** `injury_notes`: 89 short
    editorial strings.
  - Roster `status`: `Active, Inactive, Injured Reserve, Physically Unable to
    Perform, Non Football Injury, Practice Squad` — **carries reserve-list
    classes directly.**
  - **Clock:** `news_updated` epoch-ms per player; newest observed
    `2026-08-13T02:50:13Z` — same-night freshness. No per-field injury
    timestamp.
- **Identity (rostered QB/RB/WR/TE, n=954):** `sportradar_id` 98.5%,
  `rotowire_id` 97.6%, **`gsis_id` 16.0%**, `espn_id` 21.5%, `yahoo_id`
  22.4%. Practical crosswalk: sleeper `player_id` → nflverse `ff_playerids` →
  gsis; no exact governed TIBER crosswalk precedent is asserted here.
- **Coverage vs the load-bearing distinction:** injury designation
  (Sleeper-normalized) ✔; reserve lists ✔; practice status ✖; gameday
  inactives ✖; official Wed/Thu/Fri detail ✖.
- **Backfill:** none — current-state dump.
- **Failure modes:** vocabulary drift; sample-period nonpopulation or possible
  deprecation (`practice_participation` was effectively empty in this preseason
  sample); no changelog; `NA`/`DNR`/`COV` under-documented.
- **Verdict:** among evaluated candidates, this was the **only one whose owner
  affirmatively documented free automated access with an explicit rate
  budget.** Tension in the observed
  text: docs say "free for non-commercial," ToU says "personal and
  non-commercial," and **no redistribution grant was found on the enumerated
  documentation and terms surfaces**.
  **Discovery disposition — not an admission record:** no admission proposal
  is made. Whether TIBER's free/no-paywall product
  fits "non-commercial," and whether normalized-fact retention counts as
  redistribution, are routed to Sleeper/counsel. This is a technically
  promising future internal candidate only if those questions resolve.

## Candidate 6 — Commercial comparison rows (recorded, excluded from the open question)

- **MySportsFeeds**: public materials described a keyed API with injuries and
  *"Affordable and FREE
  options for API access for personal/private use"* — non-commercial access
  is registration/patronage-gated, commercial requires a paid license → fails
  "free/open, redistributable" on the commercial-license and no-redistribution
  axes.
- **SportsDataIO**: reviewed public materials described enterprise sales and a
  free trial, with no public pricing; redistribution required contractual
  rights. This comparison therefore did not qualify it as free/open.

## Discovered candidate (evaluated) — nflverse `injuries` release

- **Identity:**
  `https://github.com/nflverse/nflverse-data/releases/download/injuries/injuries_2025.csv`
  (+ `.parquet`), 200, public, no auth. **`injuries_2026.*` → 404** at the
  observed locator; this does not establish future publication behavior.
- **Observed content (2025):** 6,068 rows; **32 distinct team values and rows
  in each of REG weeks 1–18 plus WC/DIV/CON/SB** (not every team-week pair; an
  operational observation of the artifact, with no authoritative expected-row
  census against which to assess completeness); 16 columns (`season,
  season_type, game_type, team,
  week, gsis_id, position, full_name, first_name, last_name,
  report_primary_injury, report_secondary_injury, report_status,
  practice_primary_injury, practice_secondary_injury, practice_status`).
  `report_status ∈ {Out, Doubtful, Questionable, ∅}`; `practice_status ∈
  {DNP, Limited, Full, ∅}`. These are fields represented by nflverse as
  injury/practice-report content and keyed by `gsis_id`; the non-public
  acquisition package prevents this reference from establishing the ultimate
  endpoint, source/access visibility, or direct provenance for each assertion.
- **Material caveats:** historical `date_modified` **absent** from the 2025
  schema (no exposed row-level revision clock or revision history; the
  observed file is a snapshot, not evidence of the Wed/Thu/Fri progression);
  a repository-level CC-BY file exists, but no
  source-specific explanation of its application to the upstream
  injury-report material was observed. No 2026 public asset was observed at the
  reviewed release locator; this establishes neither pipeline execution nor
  future publication behavior. The 2026 continuity question remains open,
  while a 2025 public artifact was observed.
- **Verdict:** publicly reachable, free, gsis-keyed, and technically the
  strongest observed 2025 candidate. No admission proposal is made. Upstream
  rights provenance, the absent
  revision clock, and in-season 2026 continuity remain unresolved.

**Deferred (named, not evaluated):** Pro-Football-Reference/Sports-Reference,
FantasyPros, RotoWire, Yahoo injuries endpoints, Ourlads depth charts, and
ESPN team-page `?enable=injuries` variants. No rights, reliability, or
admission disposition is assigned to an unevaluated source.

---

## Comparison matrix

| Dimension | media.nfl.com PDFs | NFL.com injuries/inactives | Club sites | ESPN site API | ESPN core API | Sleeper dump | nflverse injuries release | MySportsFeeds / SportsDataIO |
|---|---|---|---|---|---|---|---|---|
| Access | **401 media gate** | Public HTML | Public HTML | Public JSON | Public JSON | Public JSON | Public CSV/parquet | Keyed / paid |
| robots posture | 403/500 (none retrieved) | Injury paths not disallowed in observed file | Sampled path not disallowed in observed file | No robots retrieved (403); www blocks `anthropic-ai` | Same | Observed `Allow: /` | GitHub | n/a |
| ToS posture | NFL ToS | **"Systematic retrieval… prohibited"** (2024-05-16) | Eagles sample matched prior Cardinals boilerplate (2026-06-01); no 32-site census | Disney ToU text reviewed; facial restrictions observed; API-host applicability unresolved (2024-05-24) | Same | **Free API, non-commercial; no redistribution** (2026-07-24) | Repository CC-BY file observed; no source-specific upstream-rights explanation found | Paid license |
| Practice status (per-day) | ✔ per-day PDFs | ✔ weekly aggregate | not established (preseason shell) | ✖ | ✖ | no usable values in sampled preseason dump; possible deprecation | represented as weekly report content; direct provenance unverified | ✔ (paid) |
| Official game designation | ✔ | ✔ | not established (preseason shell) | ~ (ESPN synthesis) | ~ | ~ (Sleeper vocab) | represented as report content; direct provenance unverified | ✔ |
| Gameday inactives | ✔ | ✔ (`/inactives/`) | not established | ✖ | ✖ | ✖ | ✖ | ✔ |
| IR/PUP/NFI reserve lists | ~ | ✖ | not established | ✔ (`fantasyStatus`) | ✔ | ✔ (roster `status`) | ✖ | ✔ |
| Player IDs | names only | names only | names only | ESPN id → gsis via ff_playerids | ESPN id | sleeper id (gsis native 16%) | **gsis native** | varies |
| Clocks in payload | per-day editions | **none** | not established (preseason shell) | ✔ top-level + per-item ISO minute | ✔ per-item | `news_updated` epoch-ms | **none** (`date_modified` gone in 2025) | ✔ |
| 2025 backfill | gated | ✔ sampled week page | not evaluated | ✖ | ✖ | ✖ | operational span incl. playoffs; completeness unproven | ✔ (paid) |
| Rate guidance | n/a | none | none | none (1 call = league) | none (N+1) | **stated: <1000/min; players 1×/day** | GitHub CDN | contractual |
| Discovery disposition — not an admission record | excluded (gated) | rights review unresolved | excluded from comparison (dominated) | rights review unresolved | same | rights review unresolved | rights/provenance/reliability review unresolved | commercial comparison; excluded from open-source scope |

## Preliminary ranking (frank)

### (a) Technical ordering for possible later admission — none admitted here

1. **nflverse injuries release** — gsis-keyed, free, with observed 2025
   practice/designation-shaped fields; blockers are unresolved upstream
   rights provenance, 2026 continuity, and the lost revision clock.
2. **Sleeper players dump** — the only owner-documented free automated feed
   among evaluated candidates;
   the preseason sample exposed current-state designation-shaped and
   reserve-list fields plus `news_updated`; blockers: "personal/non-commercial"
   ToU vs TIBER's free public product, no usable practice-status witness in the
   sample, no history, and weak native gsis.
3. **NFL.com injuries + inactives pages** — league-published content, uniquely
   covers gameday inactives; the paths were not disallowed by the observed
   robots file, but the dominant blocker is the
   unresolved admission/rights review driven by the facial
   systematic-retrieval ToS text; also no timestamps and a live routing
   anomaly.
4. **ESPN site API** — best clocks and reserve-list detail, one-call league
   snapshot; dominant blocker is the most restrictive terms text observed in
   this audit; expressive editorial comment content was not retained.
5. **ESPN core API** — same unresolved state, worse politeness economics.
6. **media.nfl.com PDFs / sampled club site** — excluded from this pass.

### (b) Upstream / nflverse contribution potential

1. **Possible hardening of the existing nflverse injuries pipeline** is the
   best-supported contribution candidate among those evaluated for fresh
   follow-up: the 2025
   asset was observed; possible work is **2026 automation continuity plus a
   `date_modified`/per-day observation clock**. The upstream rights record
   remains unresolved; a technical contribution would not resolve it.
2. **NFL.com pages as the pipeline's source** — league-published content whose
   paths were not disallowed by the observed robots file, but the observed ToS
   text facially restricts systematic retrieval; no permission determination
   is made here.
3. **ESPN and Sleeper are not supported as upstream donors by this record.**
   ESPN's observed ToU text facially restricts redistribution; Sleeper's
   observed terms are non-commercial and no redistribution permission was
   found. Their identifiers/vocabularies also require mapping.

## Unresolved questions only rights counsel or the source owner can answer

1. **NFL ToS scope:** does extraction of the underlying facts survive the
   §1.3 systematic-retrieval clause (fact protectability; browsewrap
   enforceability; hiQ/Van Buren-line reasoning)? The clause text on its
   face describes the pipeline pattern; whether it binds is the counsel
   question.
2. **Sleeper's "non-commercial":** does a free, open-source, no-paywall
   analytics product qualify? Does "reach out… to discuss licensing" apply to
   normalized facts as well as raw payloads? Only Sleeper can say (contact
   not authorized in this discovery).
3. **ESPN undocumented endpoints:** any implied license from a decade of
   serving unauthenticated JSON, against the Disney ToU and absent robots on
   the API hosts? Counsel question; no owner position was found on the reviewed
   public surfaces.
4. **nflverse chain of rights:** under what theory does nflverse-data
   redistribute official injury-report content, and does TIBER's governed
   admission inherit protection or exposure? Counsel and/or maintainers
   (outreach not authorized).
5. **media.nfl.com credentials:** would NFL grant non-newsroom research
   access to the per-day PDFs? Only NFL Communications can answer.
6. **AI-use clauses:** are agent-mediated sample reads of Disney properties
   distinguishable from the prohibited class going forward? A counsel
   determination should precede any repeat sampling of Disney properties.
